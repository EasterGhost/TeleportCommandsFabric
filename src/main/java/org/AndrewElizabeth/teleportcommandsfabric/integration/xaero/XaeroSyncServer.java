...
public final class XaeroSyncServer {
	...
	private static void onServerTick(MinecraftServer server) {
		...
		int intervalSeconds = CONFIG.getXaero().getSyncIntervalSeconds();
	...
	private static void sendSync(ServerPlayer player) {
		...
		ModConstants.LOGGER.info("Sending Xaero sync to {} (warps: {}, homes: {})", player.getName().getString(),
				payload.warps().size(), payload.homes().size());
	...
	private static XaeroSyncPayload buildPayload(ServerPlayer player) {
		...
		boolean persist = CONFIG.getXaero().isPersistWaypointSets();
		String warpSetName = CONFIG.getXaero().getWarpSetName();
		String homeSetName = CONFIG.getXaero().getHomeSetName();
		return new XaeroSyncPayload(warps, homes, persist, warpSetName, homeSetName);
	}

	private static boolean isEnabled() {
		try {
			return CONFIG.getXaero().isEnabled();
		} catch (Exception e) {
			ModConstants.LOGGER.error("Xaero sync config error", e);
			return false;
		}
	}
	...
	private static long getRequestIntervalMs() {
		try {
			int configured = CONFIG.getXaero().getSyncIntervalSeconds();
			if (configured > 0) {
				return configured * 1000L;
			}
		} catch (Exception e) {
			ModConstants.LOGGER.error("Xaero sync interval read error", e);
		}
		return ModConstants.SYNC_INTERVAL_MS;
	}
}
