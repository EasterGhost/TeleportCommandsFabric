package org.AndrewElizabeth.teleportcommandsfabric.network.core;
...
public final class XaeroSyncPackets {
	...
	public static final Identifier SYNC_REQUEST_ID = Identifier.fromNamespaceAndPath(ModConstants.MOD_ID, "xaero_sync_request");
	public static final Identifier SYNC_DATA_ID = Identifier.fromNamespaceAndPath(ModConstants.MOD_ID, "xaero_sync_data");
	...
	private static List<XaeroSyncEntry> readEntries(FriendlyByteBuf buf) {
		...
		for (int i = 0; i < size; i++) {
			entries.add(new XaeroSyncEntry(buf.readUtf(), buf.readUtf(), buf.readInt(),
