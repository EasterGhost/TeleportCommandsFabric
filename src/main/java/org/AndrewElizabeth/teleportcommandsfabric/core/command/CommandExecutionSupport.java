...
public final class CommandExecutionSupport {
	...
	public static void sendWithArgs(ServerPlayer player, String key, ChatFormatting formatting, Object... args) {
		MutableComponent[] translatedArgs = new MutableComponent[args.length];
		for (int i = 0; i < args.length; i++) {
			translatedArgs[i] = Component.literal(args[i].toString());
		}
		player.sendSystemMessage(getTranslatedText(key, player, translatedArgs).withStyle(formatting), true);
	}
	...
	public static void sendInvalidPage(ServerPlayer player, int requestedPage, int totalPages) {
		player.sendSystemMessage(
				getTranslatedText("commands.teleport_commands.common.invalidPage", player,
						Component.literal(String.valueOf(requestedPage)), Component.literal(String.valueOf(totalPages)))
	...
	public static int execute(ServerPlayer player, String errorLogMessage, String errorTranslationKey,
		...
		try {
			...
			player.sendSystemMessage(getTranslatedText(errorTranslationKey, player)
