...
final class WarpCommandSupport {
	...
	static void printWarps(CommandSourceStack source, ServerPlayer player, int page, String dimensionFilter) {
		...
		List<NamedLocation> warps = DimensionFilterCommandSupport.sortAndFilter(STORAGE.getWarps(),
				normalizedDimensionFilter);
		PagedListCommandSupport.displayPage(player, warps, page, () -> sendEmptyMessage(player, normalizedDimensionFilter),
				(requestedPage, totalPages) -> WarpMessages.sendInvalidPage(player, requestedPage, totalPages),
				(pageEntries, currentPage, totalPages) -> WarpFormatter.buildWarpList(source, player, pageEntries, currentPage,
						totalPages, normalizedDimensionFilter));
	}
	...
	static void printWarpPagePicker(ServerPlayer player, int currentPage, String dimensionFilter) {
		...
		List<NamedLocation> warps = DimensionFilterCommandSupport.sortAndFilter(STORAGE.getWarps(),
				normalizedDimensionFilter);
		PagedListCommandSupport.displayPagePicker(player, warps, currentPage,
				() -> sendEmptyMessage(player, normalizedDimensionFilter),
				(requestedPage, totalPages) -> WarpMessages.sendInvalidPage(player, requestedPage, totalPages),
				(page, totalPages) -> WarpFormatter.buildWarpPagePicker(player, page, totalPages, normalizedDimensionFilter));
	}
