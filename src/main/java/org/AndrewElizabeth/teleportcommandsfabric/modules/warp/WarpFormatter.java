...
final class WarpFormatter {
	...
	static MutableComponent buildWarpList(CommandSourceStack source, ServerPlayer player, List<NamedLocation> warps,
		...
		message.append(PaginationCommandSupport.buildHeader(player, "commands.teleport_commands.warps.title", currentPage,
				totalPages));
	...
	static MutableComponent buildWarpPagePicker(ServerPlayer player, int currentPage, int totalPages,
		...
		return PaginationCommandSupport.buildPagePicker(player,
				"commands.teleport_commands.warps.title", currentPage, totalPages,
				page -> DimensionFilterCommandSupport.buildPageCommand(WARP_LIST_COMMAND, page, dimensionFilter));
	}
	...
	private static void appendWarpEntry(MutableComponent message, CommandSourceStack source, ServerPlayer player,
		...
		String quotedName = CommandArgumentSupport.quoteCommandArgument(currentWarp.getName());
		...
		CommandUiSupport.appendNameLine(message, currentWarp.getName(), getTranslatedText(playerVisible
		...
		message.append(Component.literal("     | ").withStyle(ChatFormatting.AQUA))
				.append(CommandUiSupport.translatedButton(player, "commands.teleport_commands.common.tp", ChatFormatting.GREEN,
						new ClickEvent.RunCommand("WarpCommand " + quotedName)))
				.append(" ");

		if (canModify) {
			message.append(CommandUiSupport.translatedButton(player, "commands.teleport_commands.common.rename",
					ChatFormatting.BLUE, new ClickEvent.SuggestCommand("/renamewarp " + quotedName + " ")))
					.append(" ")
					.append(CommandUiSupport.translatedButton(player, "commands.teleport_commands.common.update",
							ChatFormatting.YELLOW, new ClickEvent.RunCommand("updatewarp " + quotedName)))
					.append(" ")
					.append(CommandUiSupport.translatedButton(player, "commands.teleport_commands.common.delete", ChatFormatting.RED,
							new ClickEvent.SuggestCommand("/delwarp " + quotedName)));
		}

		message.append(" ")
				.append(CommandUiSupport.translatedButton(player, playerVisible
						? "commands.teleport_commands.common.hideFromMap"
						: "commands.teleport_commands.common.showOnMap", playerVisible ? ChatFormatting.GRAY : ChatFormatting.GOLD,
						new ClickEvent.RunCommand("teleportcommandsfabric:mapwarp " + quotedName + " "
	...
	private static boolean isVisibleForPlayer(ServerPlayer player, UUID warpUuid) {
		return STORAGE.getPlayer(player.getStringUUID())
