...
final class WarpAdminFormatter {
	...
	private static void appendWarpEntry(MutableComponent message, ServerPlayer player, NamedLocation currentWarp) {
		...
		MutableComponent visibilityState = getTranslatedText(visible
				? "commands.teleport_commands.gwarpmap.globalVisible"
				: "commands.teleport_commands.gwarpmap.globalHidden", player)
						.withStyle(visible ? ChatFormatting.DARK_GREEN : ChatFormatting.GRAY);
		MutableComponent toggleButton = CommandUiSupport.translatedButton(player, visible
				? "commands.teleport_commands.gwarpmap.hideFromMap"
				: "commands.teleport_commands.gwarpmap.showOnMap", visible ? ChatFormatting.GRAY : ChatFormatting.GOLD,
				new ClickEvent.RunCommand("gwarpmap " + quotedName + " " + (visible ? "false" : "true")));
		MutableComponent resetButton = CommandUiSupport.translatedButton(player, "commands.teleport_commands.common.update",
