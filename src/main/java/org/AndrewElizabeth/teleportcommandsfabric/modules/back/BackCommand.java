...
public class BackCommand {
	...
	private static LiteralArgumentBuilder<CommandSourceStack> buildBackNode() {
		return Commands.literal(COMMAND_BACK)
				...
				.then(Commands.argument("Disable Safety", BoolArgumentType.bool())
						.requires(source -> source.getPlayer() != null)
						.executes(context -> handleBackDeath(context.getSource().getPlayerOrException(),
								BoolArgumentType.getBool(context, "Disable Safety"))))
				.then(buildBackModeNode(MODE_DEATH, BackCommand::handleBackDeath))
				.then(buildBackModeNode(MODE_TP, BackCommand::handleBackTp));
	}
	...
	private static int handleBackDeath(ServerPlayer player, boolean safetyDisabled) {
		return BackMessages.execute(player, "Error while going back! => ", () -> toDeathLocation(player, safetyDisabled));
	}

	private static int handleBackTp(ServerPlayer player, boolean safetyDisabled) {
		if (!BackMessages.ensureEnabled(player, CONFIG.getBack().isEnabled())) {
			return 1;
		}
	...
	private static int toDeathLocation(ServerPlayer player, boolean safetyDisabled) throws Exception {
		...
		if (deathLocationWorld == null) {
			ModConstants.LOGGER.warn(
					"([back]) Error while going back! \nCouldn't find a world with the id: \"{}\"", player.getName().getString(),
		...
		if (CONFIG.getBack().isDeleteAfterTeleport()) {
			String playerUuid = player.getStringUUID();
			onTeleportSuccess = () -> DeathLocationStorage.removeDeathLocation(playerUuid);
		}

		return BackCommandSupport.teleportResolvedLocation(player, deathLocation.getBlockPos(), deathLocationWorld,
				safetyDisabled, COMMAND_BACK_DEATH_FORCE, BackMessages::sendSame, BackMessages::sendGo, onTeleportSuccess, true);
	}
	...
	private static int toPreviousTeleportLocation(ServerPlayer player, boolean safetyDisabled) throws Exception {
		...
		if (previousTeleportWorld == null) {
			ModConstants.LOGGER.warn(
					"([back]) Error while going back to previous teleport location! \nCouldn't find a world with the id: \"{}\"",
					player.getName().getString(), previousTeleportLocation.getWorldString());
			BackMessages.sendWorldNotFound(player);
			return 0;
		}

		return BackCommandSupport.teleportResolvedLocation(player, previousTeleportLocation.getBlockPos(),
				previousTeleportWorld, safetyDisabled, COMMAND_BACK_TP_FORCE, BackMessages::sendPreviousTeleportSame,
				BackMessages::sendPreviousTeleportGo, null, false);
	}
