...
final class AdminConfigNodeFactory {
	...
	static LiteralArgumentBuilder<CommandSourceStack> intNode(String literalName, String argName, int min,
			IntSupplier getter, IntConsumer setter, String messageKey) {
				...
				.then(Commands.argument(argName, IntegerArgumentType.integer(min))
						.executes(context -> AdminMessages.setAndSave(context,
								...
								AdminMessages.t(context.getSource(), messageKey,
										AdminMessages.intArg(context, argName)))));
	}
	...
	static LiteralArgumentBuilder<CommandSourceStack> boolNode(String literalName, Supplier<Boolean> getter,
			Consumer<Boolean> setter, String messageKey) {
		return Commands.literal(literalName)
				.executes(context -> AdminMessages.sendCurrentValue(context.getSource(), literalName,
						AdminMessages.t(context.getSource(), getter.get()
								? "commands.teleport_commands.admin.stat.enabled"
								: "commands.teleport_commands.admin.stat.disabled")))
				.then(Commands.literal("true")
						.executes(context -> AdminMessages.setAndSave(context, () -> setter.accept(true),
								AdminMessages.t(context.getSource(), messageKey, AdminMessages.t(context.getSource(),
										"commands.teleport_commands.admin.stat.enabled")))))
				.then(Commands.literal("false")
						.executes(context -> AdminMessages.setAndSave(context, () -> setter.accept(false),
								AdminMessages.t(context.getSource(), messageKey, AdminMessages.t(context.getSource(),
										"commands.teleport_commands.admin.stat.disabled")))));
	}

	static LiteralArgumentBuilder<CommandSourceStack> stringNode(String literalName, String argName,
			Supplier<String> getter, Consumer<String> setter, String messageKey) {
		return Commands.literal(literalName)
				.executes(context -> AdminMessages.sendCurrentValue(context.getSource(), literalName,
				...
				.then(Commands.argument(argName, StringArgumentType.string())
						.executes(context -> {
							...
							return AdminMessages.setAndSave(context, () -> setter.accept(value),
