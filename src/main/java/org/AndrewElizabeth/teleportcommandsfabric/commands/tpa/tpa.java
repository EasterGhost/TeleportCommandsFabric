		...
		return Commands.literal(literal)
				...
				.then(Commands.argument("player", EntityArgument.player())
						.executes(context -> {
							...
							return TpaMessages.execute(errorLogMessage,
									() -> TpaService.sendRequest(player, targetPlayer, here));
						}));
	}
		...
		return Commands.literal(literal)
				...
				.then(Commands.argument("player", EntityArgument.player()).suggests(new TpaSuggestionProvider())
						.executes(context -> {
							...
							return TpaMessages.execute(errorLogMessage,
									() -> handleResponse(player, targetPlayer, null, accept));
						})
						.then(Commands.argument("requestId", StringArgumentType.word())
								.executes(context -> {
									...
									return TpaMessages.execute(errorLogMessage,
											() -> handleResponse(player, targetPlayer, requestId, accept));
								})));
	}
