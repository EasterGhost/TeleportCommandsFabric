...
```bash
/teleportcommands config rtp radius <半径>
```
...
| Home删除无效传送点 | `/teleportcommands config home deleteInvalid <true \| false>`       | 自动删除无效的家           |
...
| 随机传送半径       | `/teleportcommands config rtp radius <半径>`                        | 设置随机传送的半径         |
